...
12. **Серверний пошук/фільтр великих колекцій**: для типів, що ростуть до тисяч (наразі `mediaItem`), пошук/фільтр/пагінація — на СЕРВЕРІ через GROQ, кероване `searchParams`. Текстовий пошук — оператор `match` із токенізованим префіксом (`toMatchQuery` у `lib/media.ts`); порожній параметр = `''` (гілка `select(..., true)` = без фільтра). Підвантаження («Завантажити ще») — Server Action (`'use server'`), що повертає наступну slice-порцію; клієнт акумулює. Спільний трансформер raw→card живе в `lib/` і використовується і сторінкою, і екшеном (одне джерело правди). НЕ фетчити всю колекцію на клієнт для in-memory фільтрування.
