...
- `next.config.ts` — `images.remotePatterns` (cdn.sanity.io) + `headers()`: security-заголовки в enforce (nosniff, Referrer-Policy, X-Frame-Options, HSTS, Permissions-Policy) на всіх маршрутах та CSP у **Report-Only** (`/studio` виключено; перемкнути в enforce після runtime-смоук — див. `context/improvement-roadmap.md` → C2)
