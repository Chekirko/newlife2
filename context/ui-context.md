...
- **Сторінка /media** (`app/(front)/media/`): СЕРВЕРНИЙ пошук/фільтр (масштаб на тисячі відео). Контролери в URL: пошук (інпут `type="search"`, debounce 350мс → `router.replace`), таби категорій + дропдаун проповідника (sermons-only) → `router.push`. Перша порція (`MEDIA_PER_PAGE=12`) рендериться серверно; **«Завантажити ще»** доакумульовує наступну порцію через Server Action `loadMoreMedia` БЕЗ навігації. Лічильник «Показано X з Y» (`aria-live="polite"`). Картки — однакові висоти; теги = некліковані чіпи (картка вже `<button>`); клік → `Lightbox`. fade-in нових карток поважає `prefers-reduced-motion`. (Клієнтську Isotope-анімацію фільтрування прибрано — несумісна із серверними фільтрами.)
