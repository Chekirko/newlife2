...
- **Порядок секцій головної** (`app/(front)/page.tsx`): Hero → Найближчі події → **Остання проповідь** (`LatestSermon` — новіший `mediaItem` категорії «проповідь», обкладинка відкриває `Lightbox`; null-guard ховає секцію, якщо проповідей немає) → Хто ми (тизер місії + CTA «Дізнатися більше → /about») → Вітання пастора (реальний старший пастор із `teamMember`, без почесних) → **Розклад богослужінь** → Що ви знайдете → Новини → Свідчення → FAQ → фінальна CTA. Секцію «Про нашу церкву» (`AboutWithStats`) **прибрано** як дубль місії; повна місія/історія/статистика йдуть на /about. Розклад піднято рано — час/місце ключові для гостя. (`AboutWithStats` лишається у `components/` для повторного використання статистики на /about.)
