...
- **Секції /about** (`app/(front)/about/page.tsx`): PageHero → Хто ми+місія (градієнт-блок місії) → Наша історія (наратив + лінк /history) → У що ми віримо (grid карток із `border-t-4 border-t-primary`) → Наші цінності (картки з круглими `bg-grad`-іконками FA) → Чого очікувати (icon-list + CTA /contact) → Керівництво (лінк /team) → Статистика (`AboutWithStats`) → CTA-градієнт. Фони чергуються white/`bg-gray-100`. Контент — `lib/about-data.ts`.
