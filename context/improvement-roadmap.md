...
- 2026-06-19 — **A4 аудит** виконано (SEO/perf/security). Реалізовано обрані фікси: **S2** — екранування JSON-LD у 8 місцях (хелпер `jsonLdHtml` у `lib/utils.ts`); **SEO quick wins** — `canonical` на 11 індексованих маршрутах + OG/Twitter/`locale`/`siteName`/зображення в root `app/layout.tsx`; **C2** — security headers (enforce: nosniff, Referrer-Policy, X-Frame-Options, HSTS, Permissions-Policy) + **CSP у Report-Only** (`/studio` виключено). `next build` ✓ 34 стор., tsc ✓. **TODO C2:** після runtime-смоук-тесту (скрипти/Sanity-зображення/Google Maps/Studio) перемкнути CSP Report-Only → enforce. Не брали: P1 (FA-сабсет). Передіснуючі lint `no-explicit-any` у history/news — поза scope.
