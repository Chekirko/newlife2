...
- Session (2026-06-20): **Реорг головної (Unit 1, акцент збалансований; skills: landing-page-generator, karpathy)**. Прибрано дубль-секцію `AboutWithStats` (місія дублювала «Хто ми»; статистику перенесемо на /about); **розклад богослужінь** піднято вище за «Що ви знайдете» (час/місце рано); `OurVision` отримав тизер-CTA «Дізнатися більше → /about». Новий порядок зафіксовано в `ui-context.md`. `next build` ✓ 34 стор., tsc/eslint ✓. **TODO Unit 1:** вітання пастора досі заглушка «Іван Петренко» — замінити на реального (з /team) по підтвердженню користувача. Рік заснування = **1991** (таймлайн /history з 1992 — узгодити при /about/Unit 2). **Далі:** Unit 2 — /about (singleton `aboutPage` + сторінка + JSON-LD).
